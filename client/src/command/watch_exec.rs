...
/// Watch the Nix Store for new paths and upload them to a binary cache, while running a command.
#[derive(Debug, Parser)]
...
pub async fn run(opts: Opts) -> Result<()> {
    ...
    let mut exec_command = Command::new(sub.command[0].as_str())
      .args(&sub.command[1..])
      .spawn()?;
      ...
      select!{
        ...
        status = exec_command.wait() => {
          ...
          return match status?.code() {
            Some(0) => Ok(()),
            Some(code) => Err(anyhow!("Process exited with status code {:?}", code)),
            None => Ok(()),
          }
        },
      }
